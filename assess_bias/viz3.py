...
def plot_words(embedding, model_name, wordlist_full):
    ...
    # load x-axis
    #x = ['mand', 'kvinde'] #np.loadtxt(os.path.join("..", "output","gender_direction.csv"), delimiter=',')
    ...
    vectors = []
    for i in range(len(wordlist_full)):
        vectors.append(embedding[vectors[i]])
    # retrieve vectors
...
plot_words(model, model_name, word_list)
'''